...
import qs from "qs";
import "./ShopProductListPage.scss"
...
export default function ShopProductListPage () {
    ...
    const [request,setRequest]=useState({page:0,size:10,shopId:getLocalMerchant().shopId});
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_, record) => (
                <Space size="middle">
                    <Button size="small" type="primary" onClick={()=>{navigate("/mall/product/edit",{state:{id:record.id,from:'/mall/shop/product/list'}})}}>编辑</Button>
    ...
    return (
        <>
            <div className="header">
                <Button onClick={()=>{navigate("/mall/product/edit",{state:{from:'/mall/shop/product/list'}})}}>新增</Button>
            </div>
            <Table
                columns={columns}
                rowKey="id"
